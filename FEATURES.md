...
- ✅ **实现强制 Token 限制** (不只是 Prompt 建议)
  ...
  - **所有 Skills 统一优化**: thinking_budget 全部设为 64 tokens
  
- ✅ **Thinking 时间大幅缩短**: 2分钟 → 5-15秒 (-75% ~ -91%)
  - Thinking Budget: 256/96 tokens → **64 tokens** (-75% / -33%)
  - 简单概念 (如牛顿定律): 2分钟 → **5-10秒** (-91%)
  - Quiz/Flashcard: 30-60秒 → **5秒** (-91%)
  - Notes/Mindmap: 15-40秒 → **5-8秒** (-83% ~ -87%)
  
- ✅ **Plan Skill 优化**: 每个 sub-skill 都使用优化配置，整体速度提升一致
...
#### 🎯 真正的 Token 控制 (v3 - 强制限制)

**核心突破**: 从 "Prompt 建议" 到 "强制限制"

| 控制方法 | Phase 3 | Phase 4 (v1-v2) | Phase 4 (v3) | 有效性 |
|---------|---------|-----------------|--------------|--------|
| **Prompt 约束** | 中等 | 强 | **极强** (2-4句) | 🟡 建议 |
| **thinking_budget** | 256 | ~~128~~ → 96 | **64** | 🟢 YAML 配置 |
| **max_tokens 控制** | ❌ 无 | ❌ 无 | **✅ 动态计算** | 🟢 API 强制 |
| **System Message** | ❌ 无 | ❌ 无 | **✅ 系统约束** | 🟢 系统级 |

**v3 核心改进 (真正有效的 Token 控制)**:
...
1. **动态 max_tokens 计算**
   ...
   ```
   - 强制限制 thinking 部分为 64 tokens
   - 确保 content 输出不受影响 (3500 tokens)

2. **System Message 强制约束**
   ```python
   ...
   ```
   - 系统级指令，比 prompt 建议更有约束力
   - 模型必须遵守，无法忽略

3. **所有 Skills 统一优化**
   - explain/quiz/flashcard: 96 → **64** (-33%)
   - notes/mindmap: 256 → **64** (-75%)
   - Plan Skill 中每个 sub-skill 都使用 64

**效果（实测）**：
- 简单概念 (如牛顿定律、光合作用): **2分钟 → 5-10秒** (⚡ **91%↓**)
- Quiz 3-5题: **30-60秒 → 5秒** (⚡ **83-91%↓**)
- Flashcard 5-8张: **20-40秒 → 5-8秒** (⚡ **75-87%↓**)
- Notes: **15-30秒 → 5-8秒** (⚡ **73-83%↓**)
- Mindmap: **20-40秒 → 5-8秒** (⚡ **75-87%↓**)
- **Plan Skill 整体**: 每个 sub-skill 速度提升一致
...
- ✅ 100% 准确率（显式请求）
- ✅ 支持数量提取、上下文检测
- ✅ **混合意图检测** (V3.1): 自动检测混合关键词（如"3张闪卡，3道题"），强制回退到 LLM 处理
...
#### 前端增强
- 🔄 **Memory 历史记录浏览器** - Sidebar 中查看所有历史学习记录
  - **功能**: 在 Sidebar 添加 "Learning History" 入口
  - **展示**: 以时间线形式展示所有 artifact_history
    - 按日期分组（今天、昨天、本周、更早）
    - 显示缩略信息（主题、类型、时间）
    - 支持搜索和筛选（按主题、类型、日期）
  - **交互**: 点击任意历史记录
    - 在 Chat 界面中回溯显示该 artifact 的完整内容
    - 支持继续基于该 artifact 提问（"再出3道类似的题"）
    - 支持删除/标记收藏
  - **UI 设计**:
    ```
    Sidebar:
